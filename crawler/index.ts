...
// 获取当前页的所有卡片信息
async function get_job_card_from_page(driver: WebDriver) {
  console.log('-----get_job_card_from_page------');

  // await driver.wait(until.elementLocated(By.css('div.search-job-result')), 10000);
...
(async function main() {
  ...
  try {
    ...
    const url = `https://www.zhipin.com/web/geek/job?query=%E5%89%8D%E7%AB%AF&city=101010100`;
    ...
    do {
      await get_job_card_from_page(driver);
