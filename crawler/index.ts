...
interface JobInfo {
  ...
  key_words: string[];
  logo: string;
  is_discriminate_against: boolean;
  is_suck_blood: boolean
}

// 学历歧视关键字
const discriminate_against_words = ['985','211']
// 资本主义吸血关键字
const suck_blood_words = ['外包', '外派', '劳务派遣', '人力', '驻场', '压力']
const key_words = [...discriminate_against_words,...suck_blood_words,'web', 'app', 'html5', 'git', 'npm', 'vue', 'vue2', 'Vue3', 'react', 'js', 'javascript', ];
  ...
  try {
    ...
    for (let word of key_words) {
      ...
      if (regex.test(detail)) {
        ...
        if (discriminate_against_words.includes(word)){
          job_info.is_discriminate_against = true
        }
        if (suck_blood_words.includes(word)){
          job_info.is_suck_blood = true
        }
      }
    }
